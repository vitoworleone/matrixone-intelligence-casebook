...
本表以仓库中公开的 Matrixflow 源码快照为依据；不同部署版本和运行配置可能存在差异。

- **已实现**：当前公开版本提供对应的产品或技术能力；
- **部分具备**：有底层服务或相邻能力，但没有确认该能力已经接入目标主链路；
- **未确认**：需要结合具体部署、配置或端到端运行结果判断；
- **当前未提供**：不属于当前公开版本的产品能力范围。

“当前未提供”表示该能力不应作为当前版本的对外承诺，不影响后续版本规划。
...
| 主文本 Cross-encoder Rerank | 独立 rerank 服务当前不是 `search_rag_chunks` 的默认处理步骤。 | 未默认启用 | 不能把独立服务存在写成主文本召回已经默认执行二阶段重排。 |
...
| 查询改写 | 当前版本未提供独立的查询改写流程。 | 当前未提供 | 不要以“多轮优化”或“问题优化”作为已交付功能。 |
| HyDE / CoRAG | 当前版本未提供对应流程。 | 当前未提供 | 调研中的方法不自动迁移到 Matrixflow。 |
| 自动关键词提取 | 当前版本未提供分段级自动抽取与可编辑工作流。 | 当前未提供 | 检索中使用查询关键词，不等于自动关键词抽取。 |
| 自动问题提取 / 关联问题 | 当前版本未提供分段级生成与召回匹配流程。 | 当前未提供 | 不要与 MaxKB/RAGFlow 的关联问题混为一谈。 |
| 页面排名 | 当前版本未提供页面优先级参与最终评分的配置。 | 当前未提供 | 不能声明为可配置页面排名。 |
...
| 调研能力 | Matrixflow 对应实现 | 状态 | 表述边界 |
| --- | --- | --- | --- |
| 知识图谱 / GraphRAG | 当前公开版本不包含实体关系图、图查询和社区报告链路。 | 当前未提供 | 不作为当前版本能力对外描述。 |
| RAPTOR | 已实现确定性的文档/节/块多层索引；文件级表格命中可按 section 扩展，但不包含递归聚类、模型摘要节点或树形递归检索。 | 当前未提供 RAPTOR | 可以说“多粒度索引已实现”，不能把它直接命名为 RAPTOR。 |
| 多跳检索 | Agent 可调用检索工具，但未确认自动问题分解、循环检索或多跳编排策略。 | 未确认 | 可以说“具备供 Agent 使用的检索工具”，不能说“已实现自动多跳 RAG”。 |
| 自反思 / 自评 | 当前版本未提供检索/回答的自动反思与重试闭环。 | 当前未提供 | 不要按 Agentic RAG 完整闭环表述。 |
| 事实核验 / Verifier | 当前版本未提供 NLI、规则或检索式事实核验链路。 | 当前未提供 | 证据提供不等于已验证事实正确。 |
...
| Q&A | 未提供“一行问答块 + 问答专用召回”流程。 | 当前未提供专用模板。 |
| 表格 | Catalog 表接入、表格行聚焦与视觉表格区域处理。 | 有相关能力，但非相同模板/切块规则。 |
| 简历、书籍、法律、手册、论文、演示文稿 | 上游可解析多种文档，但当前未提供相应预设切片模板与专属召回策略。 | 不应宣称已支持 RAGFlow 同名模板。 |
| One（整篇不切） | 当前版本未提供以整份文档作为单块、依赖 LLM 长上下文的模式。 | 当前未提供。 |
