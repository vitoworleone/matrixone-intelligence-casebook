...
| --- | --- | --- | --- |
| 文件与表接入 | 本地文件、Catalog 文件、Catalog 表、来源选择；来源关系和作业持久化。 | 已实现 | 不等同于对所有外部格式或 Web 站点同步均已支持。 |
...
| 表格证据 | 依据关键词聚焦表格行。 | 已实现 | 未确认 RAGFlow 的“固定 12 行一块”策略。 |
...
| 回答引文 | 检索结果含文件、标签、视觉与证据组信息。 | 未确认 | 终端回答是否强制展示/校验引文需结合具体 Agent 与 UI 验证。 |
...
- RAGFlow 的图谱、RAPTOR、场景切片模板、Tavily 深度搜索；
- AnythingLLM 的产品介绍；
...
> Matrixflow 已具备面向文档、表格和图片知识库的版本化数据接入，以及全文与向量混合召回、证据扩展和 Agent 检索工具调用能力；来源状态、标签、有效期和索引版本参与可检索范围控制。
