...
本目录汇集 RAG 调研、产品选型判断和历史架构分析。三类内容必须区分：调研资料用于了解候选能力，策略文档用于说明投入边界，架构分析不等同于当前产品承诺，不能相互替代。

> [!NOTE]
> [!NOTE]
> 公开仓库不保留产品源码快照。调研中的外部产品能力不会自动计入任何产品的现有能力；历史架构分析需在后续实现中重新验证。
...
- [历史 RAG 架构分析](matrixflow-rag-architecture.md)：数据如何进入知识库、如何被检索、如何变成 Agent 可用证据。
- [RAG 能力映射与边界](matrixflow-rag-capability-map.md)：将调研能力映射为后续需要验证的产品边界。
