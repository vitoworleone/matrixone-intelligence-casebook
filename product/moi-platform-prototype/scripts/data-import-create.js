    ...
    // === CSV Config ===
    // nullMarkers(空值转换)默认只认 \N(保守):错转 NULL 是不可见的数据损坏,宁可导入报错;常见变体由用户显式添加
    let csvConfig = { separator: ',', delimiter: '"', escape: false, nullMarkers: ['\\N'] };
    let csvConfigConfirmed = false;
    let csvNullDraft = ['\\N'];

    function renderNullChips() {
      if (!document.getElementById('csvNullChips')) { updateCsvPreview(); return; }
      document.getElementById('csvNullChips').innerHTML = csvNullDraft.map((m, i) =>
        `<span class="csv-null-chip">${m.replace(/&/g, '&amp;').replace(/</g, '&lt;')}<span class="rm" onclick="removeNullMarker(${i})">✕</span></span>`
      ).join('');
      updateCsvPreview();
    }

    function resetCsvConfig() {
      document.getElementById('csvSeparator').value = ',';
      document.getElementById('csvDelimiter').value = '"';
      document.getElementById('csvEscape').checked = false;
      csvNullDraft = ['\\N'];
      renderNullChips();
    }
    function addNullMarker(v) {
      v = (v || '').trim();
      if (!v || csvNullDraft.includes(v)) return;
      csvNullDraft.push(v);
      renderNullChips();
    }
    function removeNullMarker(i) { csvNullDraft.splice(i, 1); renderNullChips(); }
    function csvNullInputKey(e) {
      if (e.key === 'Enter' || e.key === ',') {
        e.preventDefault();
        addNullMarker(e.target.value);
        e.target.value = '';
      } else if (e.key === 'Backspace' && !e.target.value && csvNullDraft.length) {
        removeNullMarker(csvNullDraft.length - 1);
      }
    }
    ...
    function openCsvConfigModal() {
      // 先开弹窗再回填字段:即使页面缓存与脚本版本错配导致个别新元素缺失,弹窗也能打开
      ...
      csvNullDraft = csvConfig.nullMarkers.slice();
      renderNullChips();
    }
    ...
    function confirmCsvConfig() {
      ...
      csvConfig.nullMarkers = csvNullDraft.length ? csvNullDraft.slice() : ['\\N'];
    ...
    function updateCsvPreview() {
      ...
      const sepRaw = document.getElementById('csvSeparator').value || ',';
      const delim = document.getElementById('csvDelimiter').value;
      const esc = document.getElementById('csvEscape').checked;

      const sepDisplay = sepRaw === '\t' ? '\\t' : (sepRaw === ' ' ? '␣' : escHtml(sepRaw));
      const delimDisplay = escHtml(delim || '');
      const nullDisp = escHtml(csvNullDraft[0] || '\\N');

      // 标注用 data-tag 挂在元素自身下方(纯 CSS 定位,见 .csv-tagged),不做 JS 测量
      const tok = (cls, text, tag) => `<span class="${cls}${tag ? ' csv-tagged' : ''}"${tag ? ` data-tag="${tag}"` : ''}>${text}</span>`;

      const headerCols = ['id', 'name', 'type', 'description', 'notes'];
      const headerText = headerCols.join(sepDisplay + ' ');

      // 样例行:021 , "MOI" , " database " , "\" moi \" ok" , \N
      let dataHtml = tok('val', '021') + tok('sep', sepDisplay, '分隔符');
      if (delim && esc) {
        dataHtml += tok('delim', delimDisplay) + tok('val', 'MOI') + tok('delim', delimDisplay)
          + tok('sep', sepDisplay)
          + tok('delim', delimDisplay, '定界符') + tok('val', ' database ') + tok('delim', delimDisplay)
          + tok('sep', sepDisplay)
          + tok('delim', delimDisplay) + tok('esc', '\\' + delimDisplay, '反斜杠转义') + tok('val', ' moi ') + tok('esc', '\\' + delimDisplay) + tok('val', ' ok' + delimDisplay);
      } else if (delim) {
        dataHtml += tok('delim', delimDisplay) + tok('val', 'MOI') + tok('delim', delimDisplay)
          + tok('sep', sepDisplay)
          + tok('delim', delimDisplay, '定界符') + tok('val', ' database ') + tok('delim', delimDisplay)
          + tok('sep', sepDisplay)
          + tok('delim', delimDisplay) + tok('val', ' moi ok ') + tok('delim', delimDisplay);
      } else {
        dataHtml += tok('val', 'MOI')
          + tok('sep', sepDisplay) + tok('val', 'database')
          + tok('sep', sepDisplay) + tok('val', 'moi ok');
      }
      dataHtml += tok('sep', sepDisplay) + tok('nul', nullDisp, 'NULL 值');

      document.getElementById('csvPreviewArea').innerHTML =
        `<div class="csv-preview-header">${headerText}</div>`
        + `<span class="csv-preview-header-label">表头</span>`
        + `<div class="csv-preview-data">${dataHtml}</div>`;
    }
    ...
    // CSV 配置弹窗:脚本加载即预渲染默认态(chips + 示例标注图),不依赖打开弹窗时的初始化调用
    try { renderNullChips(); } catch (e) { /* 弹窗元素缺失时忽略,HTML 内置静态默认态兜底 */ }
